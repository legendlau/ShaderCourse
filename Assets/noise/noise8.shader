Shader "Unlit/noise8"
{
    ...
    return mix( mix( dot( random2(i + vec2(0.0,0.0) ), f - vec2(0.0,0.0) ), 
			...
			{
				/*
			   float2 uv1 = i.uv * float2(1, 1);
			   //uv1 += abs(sin(noise1(_Time.y))+1);

			   uv1 += noise2(uv1*float2(1, 1))*(abs(sin(0.1*_Time.y))+1);

				float inRect = 1- drops(uv1);
