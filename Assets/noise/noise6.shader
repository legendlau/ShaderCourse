Shader "Unlit/noise6"
{
    ...
    return mix( mix( dot( random2(i + vec2(0.0,0.0) ), f - vec2(0.0,0.0) ), 
			...
			fixed4 frag (v2f i) : SV_Target
			{
				/*
			   float2 uv1 = i.uv * 10;
			   //float2 uv1 = i.uv * lerp(5, 10, abs(sin(_Time.y)));
			   ...
			   float realPos = noise3(uv1)*0.5+0.5;
			   //float inRect  = myrand(realPos);
			   float inRect = realPos;

			   fixed3 col = fixed3(inRect, inRect, inRect);

			   //fixed3 inCol  = fixed3(0.980,0.172,0.196); 
			   //fixed3 influenced_color_a = fixed3(0.880,0.793,0.581);
			   //fixed3 realCol = inCol*0.8 + col*0.1;


			   return fixed4(col, 1);
			   */

			   float2 uv1 = i.uv * float2(3, 20);
			   uv1 = rotate(noise2(uv1), uv1);
			

			   float2 uv2 = frac(uv1);
			   float2 uv3 = floor(uv1);

				float inRect = lines(uv2, uv3);
				fixed3 col = fixed3(inRect, inRect, inRect);
				return fixed4(col, 1);
			}
