    ...
    return fract(xyz);
			...
			*/
			...
			/*
				...
				//uv1 = skew(uv1);
				float width = lerp(0.1, 0.42 , (sin(_Time.y)+1)/2);

				float start2 = 0.4- width;

				//float3 col = simplexGrid(uv1);
				//col.rg = frac(uv1);
				//col.b = 0;
				float inRect1 = rectRange2(float2(-0.4, -0.1), float2(width, 0.2), uv1);
				float inRect2 = rectRange2(float2(start2, -0.1), float2(width, 0.2), uv1);
				float result = max(inRect1, inRect2);
				float3 col = float3(result, result, result);
				*/

				float cv = cellularValue(i.uv);
				float3 col = float3(cv, cv, cv);
				return fixed4(col, 1);
			}
