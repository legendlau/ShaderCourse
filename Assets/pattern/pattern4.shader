	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
			   ...
			   fixed M0 = step(fmod(colRow.y, 2), 1);

			   fixed offS = M0*_Time.y;
			   fixed offS1 = -(1-M0)*_Time.y;

			    
			   uv1 = frac(fixed2( (M0*(uv1.x+offS)+(1-M0)*( uv1.x+offS1))*3, uv1.y*3));
			   //uv1.x = uv1.x + 0.5*M0;
