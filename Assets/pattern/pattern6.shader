	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
			   ...
			   fixed index = intPart.x;
			   float PI90 = -3.14159/2;

			   fixed part0 = step(index, 0);
			   fixed part1 = (1-part0)*step(index, 1);
			   fixed part2 = (1-part0)*(1-part1)*step(index, 2);
			   fixed part3 = (1-part0)*(1-part1)*(1-part2)*step(index, 3);

			   fixed reverse = step(fmod(intPart.y, 2), 0);
			   fixed reverse2 = step(fmod(intPart.x, 2), 0);

			   fixed theta = part0*PI90 + part1*PI90*2 + part2*PI90 + part3*PI90*2+(1-reverse)*PI90*3+(1-reverse)*(1-reverse2)*PI90*2;

			   //fixed theta = part0 * PI90;
			   //fixed theta = PI90;
