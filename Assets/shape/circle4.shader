		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				*/
				...
				fixed px = 0.2*(step(0, offsetpos.x)*2-1);
				fixed py = 0.2*(step(0, offsetpos.y)*2-1);

				fixed2 newCenter = fixed2(px, py);

				fixed dist = length(offsetpos - newCenter);
				fixed dist0 = dist * 5;
				fixed dist4 = frac(dist0);

				fixed dist1 = step(0.3, dist4);
				fixed dist2 = 1-step(0.5, dist4);
				fixed dist3 = dist1 * dist2;

				fixed4 col = lerp(fixed4(0, 0, 0, 1), fixed4(1, 1, 1, 1), dist3);
				...
				*/
				...
				fixed y = abs(cos(theta*3))*0.5;

				fixed4 col = lerp(fixed4(0, 0, 0, 1), fixed4(1, 1, 1, 1), step(y, radius));
				*/
				...
				//y = tan(135) * x + b
				//0 90 
				float degree = 2.0*3.14/4.0;
				fixed deIn = floor(theta/degree);

				fixed adeg = 3*degree/2+deIn*degree;
				fixed bdeg = deIn*degree+degree/2;
				fixed y = tan(adeg)*offsetpos.x+0.4*sin(bdeg);
				fixed l = length(fixed2(offsetpos.x, y));


				fixed4 col = lerp(fixed4(0, 0, 0, 1), fixed4(1, 1, 1, 1), step(l, radius));
				return col;
			}
