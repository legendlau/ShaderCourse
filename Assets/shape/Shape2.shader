	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed px = pow(i.uv.x, 5);
				fixed px1 = pow(i.uv.x+0.1, 5)+0.1;
				fixed px2 = pow(i.uv.x-0.1, 5)-0.1;

				fixed pct1 = 1-smoothstep(px, px1, i.uv.y);
				fixed pct2 = smoothstep(px2, px, i.uv.y);
				//fixed pct = min(pct1, pct2);
				//fixed pct = pct1 - pct2;
				//fixed pct = pct1;
				fixed pct = pct1*pct2;
				//fixed pct = pct1*pct2;
