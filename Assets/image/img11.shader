...
bool PointInTriangle (fPoint pt, fPoint v1, fPoint v2, fPoint v3)
{
    ...
    return ((b1 == b2) && (b2 == b3));
			...
			*/
			...
			float fracValue4(float2 pos) {
				float num = 100;
				float2 pos1 = (pos + float2(-0.8, -0.5))*3;
				float2 c = pos1;
				float2 z = pos1;
				//zn = z * z + c
				for(int i = 0; i < num; i++) {
				...
				return i ==num ? 0.0 : float(i)/num;
				//return i == num;

			}

			float fracValue5(float2 pos) {
				float num = 20;
				float2 pos1 = (pos + float2(-0.5, -0.5))*10;
				//float2 c = pos1;
				float2 c = float2(0.3, 0.4);
				float2 z = pos1;
				//zn = z * z + c
				for(int i = 0; i < num; i++) {
					float2 zn;
				...
				//return i == num;
				return i ==num ? 0.0 : float(i)/num*2;
			}

			float2 animUV(float2 pos2) {
				float t = _Time.y*8;
				float num = floor(t);
				num = fmod(num, 20);

				float row = floor(num / 5);
				float col = num - row*5;
				...
				float xp = col * colW;
				float yp = 1 - row * rowH-rowH;

				return float2(xp, yp)+pos2 * float2(colW, rowH);
				//return pos2;
			}

			float2 adjustUV(float2 pos) {
				//989 742
				//0 - 1  ==> 989
				//0 - 1  ==> 0 ---> 1.xx 
				float2 pos2 = float2(pos.x, pos.y * 989/742);
				return pos2;
			}
			float4 invCol(float4 col) {
				float3 c = 1-col;
				return float4(c, 1);
			}

			//
			float3 convolution(float kernel[9], float3 col[9]) {
			...
			//3 * 3 
			void getData(float2 pos, out float3 ret[9]) {
				//float w = 1/1024.0;
				//float h = 1/1024.0;
				...
				for(int i = -1; i < 2; i++) {
					for(int j = -1; j < 2; j++) {
						float2 pos1 = pos + float2(i*w, j*h);
						float4 col = tex2D(_MainTex, pos1);
						ret[c] = col.rgb;
						c++;
					}
				}
        ...
        step(base, vec3(0.5))
			...
			*/
			...
			float3 softLight(float3 base, float3 lightImg) {
				float3 c1 = sqrt(base) * (2.0*lightImg - 1.0)+ 2.0 * base * (1.0-lightImg);
				float3 c2 = 2.0 * base * lightImg + base * base * (1.0 - 2.0*lightImg);
				return lerp(c1, c2, step(base, float3(0.5, 0.5, 0.5)));
			}
			/*
        ...
        tc = vec3(0.0, 0.0, 0.0);
			...
			*/
			...
			float3 hatch(float2 uv) {
				float lum = gray(tex2D(_MainTex, uv).rgb);
				float3 tc = float3(1.0, 1.0, 1.0);

				uv *= 200;
				if (lum < lum_threshold_1) 
			    {
			      if (fmod(uv.x + uv.y, 4.0) < 1.0)  {
			    ...
			    if (lum < lum_threshold_3) 
			    {
			      if (fmod(abs(uv.x + uv.y - hatch_y_offset), 4.0) < 1.0) 
			        tc = float3(0.0, 0.0, 0.0);
			    }  
    ...
    return newColor;
			...
			*/
			...
			//_MainTex image
			//_MainTex2  lookup table  8*8
			float3 lookup(float3 colo) {
				float3 fixCol = float3(colo);
				//fixCol.b = 0.03125;
				//fixCol.g = 1;

				float b = fixCol.b*63.0;
				float rate = frac(b);


				//float2 quad1 = float2(0, 0);
				float row = floor(floor(b)/8.0);
				float col = floor(floor(b) - 8.0*row);
				...
				float row1 = floor(ceil(b)/8.0);
				float col1 = floor(ceil(b)-8.0*row1);
				//float2 quad2 = float2(col1, row1);

				float2 texPos1;
				texPos1.y = row*0.125+(2.0/512.0)+fixCol.g*0.9*(0.125-2.0/512.0);
				texPos1.x = col*0.125+(2.0/512.0)+fixCol.r*0.9*(0.125-2.0/512.0);
				texPos1.y = 1-texPos1.y;

				float2 texPos2;
				texPos2.y = row1*0.125+(2.0/512.0)+fixCol.g*0.9*(0.125-2.0/512.0);
				texPos2.x = col1*0.125+(2.0/512.0)+fixCol.r*0.9*(0.125-2.0/512.0);
				texPos2.y = 1-texPos2.y;

				float4 colo1 = tex2D(_MainTex1, texPos1);
				float4 colo2 = tex2D(_MainTex1, texPos2);
				//return colo1;

				//float4 colo2 = float4(colo, 1);
				//return float3(texPos1, 0);
				return lerp(colo1, colo2, rate).rgb;
				//return float3(rate, 0, 0);
				//return colo;
				//return float3(texPos1, 0);
			}
