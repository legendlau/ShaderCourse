...
bool PointInTriangle (fPoint pt, fPoint v1, fPoint v2, fPoint v3)
{
    ...
    return ((b1 == b2) && (b2 == b3));
			...
			*/
			...
			{
				//float2 ani = animUV(i.uv);
				float2 pos = i.uv; 
				float4 col1 = tex2D(_MainTex, pos);
				float4 col2 = tex2D(_MainTex1, pos);
				//float3 col = col1 + col2;
				//float3 col = col1 * col2;
				//float3 col = col1 - col2;
				//float3 col = abs(col1 - col2);
				//float3 col = col1/col2;
				//float3 col = max( col1, col2);
				float3 col = min( col1, col2);
